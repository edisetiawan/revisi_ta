...
$tabel_row=$_POST['pilihan'];
$keyword=$_POST['keyword'];
//echo $pilihan."<br>".$keyword; exit;
$batas=12;
...
$arr=tampilBuku();    
  ...
  <?php  } ?>
                        ...
                        INNER JOIN buku 
                            ON (buku.kode_buku = dikarang.kode_buku) WHERE nama_pengarang LIKE '%".$keyword."%'; ";// where  LIMIT $posisi,$batas");
...
FROM
    penerbit
    INNER JOIN buku 
        ON (penerbit.penerbit_kode = buku.kode_penerbit) where ".$tabel_row."  like '%".$keyword."%'  ";// where  LIMIT $posisi,$batas");
                        $result=mysql_query($query);
                        //echo $query; exit;
                        $rows=mysql_num_rows($result);
                        if($rows == 0){
